...
function loadAdditionalContent()
{
	...
	if ($rxb == false)	//If false then there is an error or query is for an instructor's courses.
	{
		...
		{
			$rxa = $dbapp->queryCourseInstructAll(34);	// 
			if ($rxa == false)
			{
				if ($herr->checkState())
					handleError($herr->errorGetMessage());
	...
	foreach ($rxa as $kx => $vx)
	{
		...
		$tdata = array(
			...
			$vx['section'],
			$vx['name'],
			$rxb['name'], //Obtained column name as well as value in queryContact
		);
...
function databaseLoad()
{
	...
	$rxa = $dbapp->queryCourse("name");	// XXX: Set This
	if ($rxa == false)
	{
		if ($herr->checkState())
...
function updateRecordAction()
{
	...
	$result = $dbapp->queryCourse("name");	// XXX: Set This
	if ($result == false)
	{
		if ($herr->checkState())
			handleError($herr->errorGetMessage());
